...
const { getTeamImagePath } = require('./team-assets');
...
function loadImageData(filePath) {
  ...
  const webpMime = ext === 'webp' ? 'image/webp' : null;
  if (!mime && !webpMime) return null;
  try {
    const base64 = fs.readFileSync(filePath).toString('base64');
    return `data:${mime || webpMime};base64,${base64}`;
  } catch {
    return null;
  }
}
...
function buildProposalSections(proposal) {
  return [
    ...
    {
      title: proposal.team?.title || 'About Our Team',
      body: [proposal.team?.intro || 'Meet the people driving the partnership.'],
      teamMembers: proposal.team?.members || []
    },
...
function addTeamSlide(pptx, teamMembers, publicDir, intro) {
  const slide = pptx.addSlide();
  slide.background = { color: THEME.bgDark };

  slide.addText('GDBIT PROPOSAL', {
    x: 0.5,
    y: 0.35,
    w: 9,
    h: 0.3,
  ...
  const cardWidth = 3.02;
  const cardGap = 0.18;
  const startX = 0.48;
  const cardTop = 1.9;
  teamMembers.slice(0, 3).forEach((member, index) => {
    const x = startX + index * (cardWidth + cardGap);
    const imagePath = getTeamImagePath(member.id, publicDir);
    ...
    slide.addShape(pptx.ShapeType.roundRect, {
      ...
      h: 3.45,
      rectRadius: 0.12,
      fill: { color: '0C1D12', transparency: 5 },
      line: { color: '274E2E', pt: 1.2 }
    });
    ...
    if (imagePath) {
      slide.addShape(pptx.ShapeType.roundRect, {
        ...
        rectRadius: 0.08,
        fill: { color: 'D4D4D4' },
        line: { color: 'D4D4D4', transparency: 100 }
      });
...
async function buildPptx(proposal, galleries, publicDir) {
  ...
  buildProposalSections(proposal).forEach((section) => {
    if (section.teamMembers) {
      addTeamSlide(pptx, teamMembers, publicDir, section.body[0]);
      return;
    }
...
async function buildDocx(proposal, galleries, publicDir) {
  ...
  buildProposalSections(proposal).forEach((section, index) => {
    ...
    if (section.teamMembers) {
      children.push(bodyParagraph(section.body[0]));
      section.teamMembers.forEach((member) => {
        children.push(
          new Paragraph({ text: member.name, heading: HeadingLevel.HEADING_2, spacing: { before: 200, after: 80 } })
        );
        if (member.position) children.push(bodyParagraph(member.position));
        if (member.bio) children.push(bodyParagraph(member.bio));
      });
      return;
    }
...
function buildPdf(proposal, galleries, publicDir) {
  ...
  buildProposalSections(proposal).forEach((section, index) => {
    if (section.teamMembers) {
      ...
      section.teamMembers.forEach((member) => {
        content.push({ text: member.name, style: 'subheader', margin: [0, 12, 0, 4] });
        if (member.position) content.push({ text: member.position, margin: [0, 0, 0, 6], bold: true, color: '#c9a227' });
        if (member.bio) content.push({ text: member.bio, margin: [0, 0, 0, 10], color: '#444' });
      });
      return;
    }
    if (section.gallery === 'military') {
